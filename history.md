...
### 폴드7 펼침 화면 레이아웃 붕괴 수정

**증상**

- 갤럭시 폴드7 펼침 화면에서 팔레트가 좌측에 세로로 걸리고 도구바/상태바가 사라져 편집 불가
- 접힌 화면은 정상 동작

**원인**

- 기본 규칙 `#app, .pm-root { display: flex; ... }` 에서 `#app` 부분이 ID 특이도(0,1,0,0)를 가진다
- 미디어 쿼리의 `.pm-root { display: grid; ... }` 는 클래스 특이도(0,0,1,0) 라서 ID 규칙에 밀려 `display: grid` 가 적용되지 않음
- 결과: 펼침 조건에서도 부모가 flex-column 으로 유지되어 `grid-area` 가 무시되고, `.pm-color-strip` 에만 걸린 `width: 56px` 때문에 좌측에 세로 스트립으로 표시됨. 도구바/상태바는 flex-column 뒤쪽 순서라 캔버스가 커진 만큼 뷰포트 밖으로 밀려 안 보임

**변경 사항**

- `index.html` 미디어 쿼리 선택자를 `.pm-root` 에서 `#app, .pm-root` 로 변경해 기본 규칙과 특이도를 맞춰 그리드 레이아웃이 실제로 적용되도록 수정
- 주석에 ID 특이도 이슈 배경을 남겨 동일 패턴 재발을 방지
